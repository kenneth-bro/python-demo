...
class Invoice:
        ...
        try:
            with pdfplumber.open(pdf_file) as pdf:
                for page in pdf.pages:
                    invoice = Invoice()
                    # 读取第一页
                    first_page = pdf.pages[page.page_number - 1]
                    # 读取文本
    ...
    def export_to_excel(invoices: "list[Invoice]", output_path: str):
        ...
        try:
            ...
            sheet.append(
                ['开票日期', '发票代码', '发票号码', '校验码', '购买方名称', '购买方税号', '销售方名称', '销售方税号',
                 '价格', '税率', '税额', '价税合计', '备注'])
            for i in range(row):
                invoice = invoices[i]
                sheet.append([invoice.date, invoice.code, invoice.number, invoice.check_code, invoice.purchaser_name,
                              invoice.purchaser_tax_number, invoice.seller_name, invoice.seller_tax_number,
                              invoice.total_amount, invoice.tax_rate, invoice.total_tax, invoice.total, invoice.remark])
...
if __name__ == '__main__':
    # 发票所在文件夹，使用绝对路径
    path = r'/Users/kenneth/Desktop/TMP/发票/me发票文件/郑-23.10.18/货拉拉电子发票/'
    # 输出文件路径
    ...
    for item in lists:
        if item.endswith('.pdf'):
            ...
            invoiceList = Invoice.read_invoice(item)
    ...
    # 合并发票到一个pdf
    # Invoice.merge_pdf(invoice_paths,
    #                   f'{output_path}/merged_invoice_{time.strftime("%Y%m%d_%H%M%S", time.localtime())}.pdf')